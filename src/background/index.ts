import { BackgroundMessage, ParamsType } from "../background-interface"
...
chrome.runtime.onMessage.addListener((message: BackgroundMessage, sender, callback) => {
  if (sender.url) {
    ...
    if (message.method === "apiGet") {
      sendUrlOnly(hostname, "GET", message.path, message.params).then(callback)
    } else if (message.method === "apiPost") {
      sendFormData(hostname, "POST", message.path, message.params).then(callback)
    } else if (message.method === "apiPatch") {
      sendFormData(hostname, "PATCH", message.path, message.params).then(callback)
    } else if (message.method === "apiDelete") {
      sendUrlOnly(hostname, "DELETE", message.path).then(callback)
    }
  }
  return true
})
