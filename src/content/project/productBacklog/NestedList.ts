export namespace NestedList {
  ...
  export const nest = <H, T>(
    ...
    list.forEach((item) => {
      ...
      const id = head ? callbacks.headId(head) : ""
      if (!store.has(id)) {
    ...
    const subLists = Array.from(store.values()).sort(
      (c1, c2) => callbacks.sortKey(c1.head) - callbacks.sortKey(c2.head)
    )
    return { subLists }
  }
  ...
  const toLocation = (arg: [subListId: string, index: number]): Location => ({
    subListId: arg[0],
    index: arg[1]
  })

  export const Move = (
    source: [subListId: string, index: number],
    destination: [subListId: string, index: number]
  ): MoveAction => ({
    id: "Move",
