...
import { DragAndDropProvider, Droppable } from "../../ui/DragAndDrop"
...
const PBISubList: React.FC<PBISubListProps> = (props) => {
  ...
  const canOver =
    ...
    (dragging: DragItem): boolean => {
      if (dragging.subListId === subList.id) {
        const diff = index - dragging.index
        if (diff === 0 || diff === 1) {
          return false
        }
    ...
    <SL>
      ...
      <SLBody>
        {subList.items.map((issue, index) => (
          <Droppable key={issue.id} item={{ index, subListId: subList.id }} canOver={canOver(index)}>
        ...
        <Droppable
          ...
          canOver={canOver(subList.items.length)}
        />
      </SLBody>
    </SL>
  )
